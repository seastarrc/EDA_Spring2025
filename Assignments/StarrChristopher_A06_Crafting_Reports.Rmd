...
```{r project.setup, results='hide', message=FALSE, warning=FALSE}
#Import libraries
library(tidyverse)
library(lubridate)
library(here)
...
```{r ozone_2018, echo=FALSE, fig.align='left', fig.height=3.5, fig.width=7, fig.cap="Distribution of Ozone Levels by Month in 2018", warning=F}
# create 2018 Ozone levels by month boxplot
epa_data %>%
  filter(year(Date) == 2018) %>%
  ggplot(aes(x = factor(month(Date)), y = Ozone)) +
  geom_boxplot() +
  labs(x = "Month", y = "Ozone Concentration (μg/m3)") +
  theme_minimal()
```
```{r ozone_2019, echo=FALSE, fig.align='left', fig.height=3.5, fig.width=7, fig.cap="Distribution of Ozone Levels by Month in 2019", warning=F}
# create 2019 Ozone levels by month boxplot
epa_data %>%
...
https://github.com/seastarrc/EDA_Spring2025
